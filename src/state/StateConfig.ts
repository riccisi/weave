...
class GlobalRegistry {
    ...
    registerMapperFactory(f: MapperFactory) {
        this.mapperFactories.set(f.name(), f);
    }

    registerAliasResolver(r: AliasResolver) {
        this.aliasResolvers.push(r);
    }

    getMapper(name: string, args: string[]) {
        return this.mapperFactories.get(name)?.create(args);
    }

    getAliasResolvers(): AliasResolver[] {
        return this.aliasResolvers.slice();
    }

    reset() {
        this.mapperFactories.clear();
        this.aliasResolvers = [];
    }
}
...
class IdentityMapper implements Mapper<any, any> {
    read(v: any) {
        return v;
    }
...
class BoolMapper implements Mapper<any, boolean> {
    read(v: any) {
        return !!v;
    }
...
class NotMapper implements Mapper<any, boolean> {
    read(v: any) {
        return !v;
    }
...
class IdentityFactory implements MapperFactory {
    name() {
        return 'id';
    }
...
class BoolFactory implements MapperFactory {
    name() {
        return 'bool';
    }
...
class NotFactory implements MapperFactory {
    name() {
        return 'not';
    }

    create() {
        return new NotMapper();
    }
}
