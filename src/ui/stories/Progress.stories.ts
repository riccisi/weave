...
export const SemanticColors: Story = {
  render: () => {
    ...
    (['default','primary','secondary','accent','info','success','warning','error'] as const)
      .forEach((color) => {
        wrap.appendChild(
          mountComponent(progress({ value: 50, color, labelMode: 'inside', thicknessClass: 'h-4', widthClass: 'w-56' }))
        );
      });
    return wrap;
  }
...
export const StripedAndAnimated: Story = {
  render: () => {
    ...
    wrap.appendChild(mountComponent(progress({ value: 70, striped: true, animated: true, color: 'success', widthClass: 'w-56' })));
    return wrap;
  }
};
